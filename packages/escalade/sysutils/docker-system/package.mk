...
PKG_DEPENDS_TARGET="toolchain sqlite go:host containerd-system runc-system btrfs-progs"
...
unpack() {
  tar -C $BUILD -zxf $SOURCES/$PKG_NAME/docker-system-$PKG_VERSION.tar.gz
  mv $BUILD/docker-$PKG_VERSION $BUILD/$PKG_NAME-$PKG_VERSION
}
...
makeinstall_target() {
  ...
  # containerd
  cp -P $(get_build_dir containerd-system)/bin/containerd $INSTALL/usr/sbin/docker-containerd
  cp -P $(get_build_dir containerd-system)/bin/containerd-shim $INSTALL/usr/sbin/docker-containerd-shim
  cp -P $(get_build_dir containerd-system)/bin/ctr $INSTALL/usr/sbin/docker-containerd-ctr

  # runc
  cp -P $(get_build_dir runc-system)/bin/runc $INSTALL/usr/sbin/docker-runc
}
